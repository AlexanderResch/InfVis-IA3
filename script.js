...
function updateDetailPanel(countryName) {
  ...
  const maleCount = rows.filter(r => r.sex === "M").length
  const femaleCount = rows.filter(r => r.sex === "F").length
  const testedCount = rows.filter(r => r.tested === "Yes").length
  const unknownTestedCount = rows.filter(r => r.tested === "Unknown").length
